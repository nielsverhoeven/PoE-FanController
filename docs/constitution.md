...
> J8 (`Custom:ESP32-P4-PoE-ETH-PinSocket`, stored in `hardware/kicad/footprints/Custom.pretty/`; footprint file renamed from `PinSocket_2x20_P2.54mm_P15.38mm_Vertical.kicad_mod` by Amendment v4.2.1) is placed along the **centre-left** of the daughter board (x = 15–36 mm), offset 15 mm from the left edge, running the full 78 mm height to mate with the Waveshare SKU 32088's male 2×20 header.
...
> - **Row A (odd pins)**: at x = 17.81 mm from the left board edge (updated from 2.81 mm by Amendment v4.3.0)
> - **Row B (even pins)**: at x = 33.19 mm from the left board edge (updated from 18.19 mm by Amendment v4.3.0)
